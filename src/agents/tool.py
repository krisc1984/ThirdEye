...
def _parse_function_tool_json_input(*, tool_name: str, input_json: str) -> dict[str, Any]:
    ...
    try:
        parsed = _load_function_tool_json_with_repair(input_json) if input_json else {}
    except Exception as exc:
        if _debug.DONT_LOG_TOOL_DATA:
...
def _load_function_tool_json_with_repair(input_json: str) -> Any:
    try:
        return json.loads(input_json)
    except Exception as first_error:
        repaired = _repair_function_tool_json_input(input_json)
        if repaired is not None and repaired != input_json:
            try:
                parsed = json.loads(repaired)
            except Exception:
                pass
            else:
                logger.debug("Repaired malformed function tool JSON arguments before parsing.")
                return parsed

        try:
            literal_parsed = ast.literal_eval(input_json)
        except Exception:
            raise first_error
        else:
            if isinstance(literal_parsed, dict):
                logger.debug("Recovered malformed function tool arguments via literal_eval fallback.")
                return literal_parsed
            raise first_error


def _repair_function_tool_json_input(input_json: str) -> str | None:
    candidate = input_json.strip()
    if not candidate:
        return None
    ...
    if first_object != -1 and last_object != -1 and first_object < last_object:
        candidate = candidate[first_object : last_object + 1]
