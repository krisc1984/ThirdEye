...
class OpenAIChatCompletionsModel(Model):
    ...
    def _merge_headers(self, model_settings: ModelSettings):
        ...
        if extra_headers is None or isinstance(extra_headers, Omit):
            extra_headers = {}
        elif not isinstance(extra_headers, Mapping):
            raise UserError("Chat Completions extra headers must be a mapping.")

        override_headers = HEADERS_OVERRIDE.get()
        if override_headers is None or isinstance(override_headers, Omit):
            override_headers = {}
        elif not isinstance(override_headers, Mapping):
            raise UserError("Chat Completions header overrides must be a mapping.")
