...
class OpenAIResponsesModel(Model):
    ...
    def _merge_headers(self, model_settings: ModelSettings):
        ...
        if extra_headers is None or _is_openai_omitted_value(extra_headers):
            extra_headers = {}
        elif not isinstance(extra_headers, Mapping):
            raise UserError("Responses API extra headers must be a mapping.")

        override_headers = _HEADERS_OVERRIDE.get()
        if override_headers is None or _is_openai_omitted_value(override_headers):
            override_headers = {}
        elif not isinstance(override_headers, Mapping):
            raise UserError("Responses API header overrides must be a mapping.")
