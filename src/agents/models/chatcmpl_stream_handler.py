...
class ChatCmplStreamHandler:
    @staticmethod
    def _extract_tool_call_name(tc_delta: Any) -> str:
        ...
        direct_name = getattr(tc_delta, "name", None)
        if isinstance(direct_name, str) and direct_name:
            return direct_name

        if isinstance(tc_delta, dict):
            function_payload = tc_delta.get("function")
            if isinstance(function_payload, dict):
                function_name = function_payload.get("name")
                if isinstance(function_name, str) and function_name:
                    return function_name
            direct_name = tc_delta.get("name")
            if isinstance(direct_name, str) and direct_name:
                return direct_name
    ...
    @staticmethod
    def _extract_tool_call_arguments(tc_delta: Any) -> str:
        tc_function = getattr(tc_delta, "function", None)
        if tc_function is not None:
        ...
        direct_arguments = getattr(tc_delta, "arguments", None)
        if isinstance(direct_arguments, str) and direct_arguments:
            return direct_arguments

        provider_specific_fields = getattr(tc_delta, "provider_specific_fields", None)
        if isinstance(provider_specific_fields, dict):
            for key in ("arguments", "tool_arguments", "function_arguments"):
                value = provider_specific_fields.get(key)
        ...
        if isinstance(tc_delta, dict):
            ...
            direct_arguments = tc_delta.get("arguments")
            if isinstance(direct_arguments, str) and direct_arguments:
                return direct_arguments
            provider_specific_fields = tc_delta.get("provider_specific_fields")
            if isinstance(provider_specific_fields, dict):
                for key in ("arguments", "tool_arguments", "function_arguments"):
                    value = provider_specific_fields.get(key)
                    if isinstance(value, str) and value:
        ...
        async for chunk in stream:
            ...
            if delta.tool_calls:
                for tc_delta in delta.tool_calls:
                    ...
                    tc_arguments = cls._extract_tool_call_arguments(tc_delta)
                    tc_name = cls._extract_tool_call_name(tc_delta)

                    # Accumulate arguments as they come in
                    state.function_calls[tc_delta.index].arguments += tc_arguments

                    # Set function name directly (it's correct from the first function call chunk)
                    if tc_name:
                        state.function_calls[tc_delta.index].name = tc_name
