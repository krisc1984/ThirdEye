...
TOOLS = [
    ...
    {{"name": "write_file_chunk", "description": "Write one text chunk to file; first chunk overwrite, later chunks append",
     "input_schema": {{"type": "object", "properties": {{"path": {{"type": "string"}}, "content": {{"type": "string"}}, "mode": {{"type": "string", "enum": ["overwrite", "append"]}}}}, "required": ["path", "content"]}}}},
    {{"name": "replace_in_file", "description": "Replace exact text in file",
...
Levels:
  ...
  1  Basic (~200 lines)  - 4 core tools: bash, read, chunked write, replace
